...
const DatesSelectStep = ({
  ...
  const disabledDays=new Array(new Date(new Date().setDate(new Date().getDate()+1)),new Date(new Date().setDate(new Date().getDate()+2)));
  const initialvalue= new Date()
  const [startDate, setStartDate] = useState(new Date());
  const [endDate, setEndDate] = useState(null);
  const [hideReturnCalendar, setHideReturnCalendar] = useState(true);
  const [disabledDate, setDisabledDate] = useState(new Array( new Date().setDate(new Date().getDate()+1), new Date().setDate(new Date().getDate()+1), new Date().setDate(new Date().getDate()+3)));
      ...
      <div className="space-y-2">
        ...
        {console.log(endDate+'--------endDate---------')}
          <div className="w-full lg:w-1/2 xl:w-1/3 space-y-3">
            <h2 className="text-xs uppercase text-gray-500">{console.log(disabledDate)+'============State disablcmded'}Date de départ</h2>
            <DatePicker
      selected={startDate}
      onChange={onChange}
      startDate={startDate}
